...
call utilities.scatter_by_clade as scatter_by_clade  {
  input:
    ...
    unique_clusters = split_by_declared_cluster.unique_clusters,
    clust_idx = pair.right
}

call clade_analysis.clade_analysis as clade_analysis  {
  input:
    cluster_name = select_first(["~{cluster_name + '_' + scatter_by_clade.cluster + '_'}clade", "~{cluster_name + '_' + pair.right + '_'}clade"]),
    filter_perc = filter_perc,
